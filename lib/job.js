...
function _addTasksWithLen(info, db_entry, storage, len, split_size) {
    ...
    {
        var instance_id_ = (n % instances) + 1;
        no_of_instances.push(instance_id_);
    }
    instanceInfo[db_entry.id.concat('_',db_entry.id.current_step)] = no_of_instances;
 ...
 */
function getTask(client,job, storage ,res)
{
    //TODO update the prev jobs of client
    client.prev_jobs.set(job);

    //get the instance from the map
    var key = job.id.concat('_',job.current_step);
...
module.exports.schedule = function(client,storage,res) {
    ...
    var prev_jobs_ = client.prev_jobs;

    if(prev_jobs_.length < 1) {
        storage.Job.findOne({
            order: [Sequelize.fn('RANDOM')]
        }).then(function (job) {
            getTask(client,job, storage, res);
        });
    }
    else
    {
        var job = prev_jobs_.pop();
        getTask(client,job, storage, res);
    }
};
